...
function renderTable(ranking_data){
  ...
  for (let i=squadra; i==squadra; i++) {
    ...
    for (let j = 0; j<(ranking[i].scores).length; j++) {
      //CHANGES COLOR DEPENDING ON JOLLY AND OPENS WINDOW ON CLICK
      let cell = jQuery("<th>", {class: getColor(solved[ranking[i].id][j]).toString() +((jollies[ranking[i].id]==j) ? " jolly":"") } );
      cell.on("click", function() {
                            if((now<countDownDate)&&(now-startDate>0)&&(solved[i][j]!=1))
                              window.open( "sendpage.php?idgara="+idgara+"&squadra="+i+"&problema="+j ,'_blank', "width=450,height=220,menubar=no,top=200");
                          });
      cell.append(ranking[i].scores[j]);
      row.append(cell);
    }
    tb.append(row);
  }
